...
-- This script is designed to work with Supabase PostgreSQL
-- Run this in the Supabase SQL Editor

-- === STEP 1: Required extension ===
-- Note: pgcrypto is usually already enabled in Supabase
...
-- Change job_cart_status column to enum safely
-- Note: Supabase might need this to be done in steps if there's existing data
DO $$
BEGIN
...
-- 3.2 quotation: change quotation_status to enum
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
...
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER -- Important for Supabase
AS $$
DECLARE
