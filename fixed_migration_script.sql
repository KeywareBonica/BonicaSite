...
-- 3.1 job_cart: add accepted_quotation_id and change job_cart_status to enum
...
-- Change status column to enum (safely)
ALTER TABLE public.job_cart
...
-- 3.2 quotation: change status column to enum
...
-- Add booking quotation foreign key constraint
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- 3.4 Create quotation_history table if missing
...
BEGIN
  IF (TG_OP = 'UPDATE') AND NEW.quotation_status = 'accepted' AND OLD.quotation_status IS DISTINCT FROM 'accepted' THEN
    -- Prevent multiple accepted quotations
