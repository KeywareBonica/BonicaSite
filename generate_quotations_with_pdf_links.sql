-- Generate quotations with actual PDF file paths
-- This script creates job carts and quotations with real PDF URLs for download
...
-- Step 2: Get existing IDs for foreign key relationships
...
-- Step 3: Create job carts for October 10, 2025 with real service data
...
-- Step 4: Generate quotations with actual PDF file paths
...
SELECT
    ...
    CASE 
        ...
        ELSE '09:30:00'::TIME
    END,
    -- Actual PDF file paths (these would be real URLs in production)
    'https://storage.googleapis.com/bonica-quotations/quotation_' || 
...
-- Step 5: Create additional quotations for variety (multiple quotations per service)
INSERT INTO quotation (
    quotation_id,
    job_cart_id,
    service_provider_id,
...
SELECT
    ...
    '2025-10-10',
    '19:00:00',
    -- Alternative PDF file paths
    'https://storage.googleapis.com/bonica-quotations/premium_quotation_' || 
    SUBSTRING(jc.job_cart_id::text, 1, 8) || '_' || 
...
-- Step 7: Show sample quotations with PDF links
SELECT 
    'Sample Quotations with PDF Links' as info,
    q.quotation_id,
    s.service_name,
