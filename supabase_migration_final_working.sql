-- === SUPABASE MIGRATION SCRIPT - FINAL WORKING VERSION ===
-- This script handles existing data values properly

-- === STEP 1: Create ENUM types with all possible values ===
DO $$
BEGIN
  -- Job Cart Status ENUM - include all possible existing values
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'job_cart_status_enum') THEN
    CREATE TYPE job_cart_status_enum AS ENUM (
      ...
      'quotation_accepted',
      'accepted',  -- Add this for existing data
      'completed',
      'cancelled'
    );
  END IF;

  -- Quotation Status ENUM - include all possible existing values
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'quotation_status_enum') THEN
    CREATE TYPE quotation_status_enum AS ENUM (
      ...
      'rejected',
      'withdrawn',
      'submitted'  -- Add this for existing data
    );
  END IF;
...
-- === STEP 2: Update existing data to match enum values ===
-- Update job_cart_status values to match enum
UPDATE public.job_cart 
SET job_cart_status = 'quotation_accepted' 
WHERE job_cart_status = 'accepted';

-- Update quotation_status values if needed
UPDATE public.quotation 
SET quotation_status = 'pending' 
WHERE quotation_status = 'submitted';
