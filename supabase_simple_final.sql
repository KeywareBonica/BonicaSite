-- === SIMPLE SUPABASE MIGRATION - NO ENUM CONVERSION ===
-- This adds the functionality you need without converting existing columns to enums
-- Run this in Supabase SQL Editor - it should work without errors!

-- === STEP 1: Add new columns to job_cart ===
...
-- === STEP 2: Add quotation_id to booking table ===
...
-- === STEP 4: Create quotation_history table ===
...
-- === STEP 5: Add unique constraint for one accepted quotation per job cart ===
-- This works with text status values
DO $$
BEGIN
  IF NOT EXISTS (
...
-- === STEP 9: Set up Row Level Security for quotation_history ===
...
-- === DONE! ===
-- This script adds all the functionality you need:
-- ✅ Proper quotation-to-job-cart linking
-- ✅ Automatic booking creation when quotations are accepted
-- ✅ Business rule: only one accepted quotation per job cart
