-- Create the missing decline_job_cart_concurrent function
-- This function handles concurrent job cart declining with proper locking
...
-- Also create the accept_job_cart_concurrent function if it doesn't exist
...
-- Create the job_cart_acceptance table if it doesn't exist
create table if not exists job_cart_acceptance (
...
create index if not exists idx_job_cart_acceptance_job_cart on job_cart_acceptance(job_cart_id);
create index if not exists idx_job_cart_acceptance_provider on job_cart_acceptance(service_provider_id);
create index if not exists idx_job_cart_acceptance_status on job_cart_acceptance(acceptance_status);

-- Add job_cart_status column if it doesn't exist
do $$ 
begin
  if not exists (select 1 from information_schema.columns 
                where table_name = 'job_cart' and column_name = 'job_cart_status') then
...
-- Create index for job_cart_status if it doesn't exist
create index if not exists idx_job_cart_status on job_cart(job_cart_status);

-- Create function to update job cart status when quotations are uploaded
...
do $$
begin
  if not exists (select 1 from pg_trigger where tgname = 'trigger_update_job_cart_status') then
