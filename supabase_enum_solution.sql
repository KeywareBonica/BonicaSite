-- === SUPABASE ENUM SOLUTION ===
-- This creates enums with ALL the values from your existing data
-- Run this in Supabase SQL Editor

-- === STEP 1: Create enums with comprehensive values ===
-- These enums include ALL possible values from your existing data
DO $$
BEGIN
  -- Job Cart Status ENUM - comprehensive values
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'job_cart_status_enum') THEN
    CREATE TYPE job_cart_status_enum AS ENUM (
      'pending',
      'accepted',                    -- Your existing value
  ...
  -- Quotation Status ENUM - comprehensive values
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'quotation_status_enum') THEN
    CREATE TYPE quotation_status_enum AS ENUM (
      ...
      'rejected',
      'withdrawn',
      'submitted',                   -- Common value
      'confirmed'                    -- Your existing value
    );
  END IF;
...
-- === STEP 3: Add new columns first (before conversion) ===
...
-- === STEP 13: Recreate quotation_with_files view ===
...
-- === DONE! ===
-- You now have:
-- ✅ Proper ENUM types with data validation
-- ✅ All the new columns you wanted
-- ✅ Quotation-to-job-cart linking
