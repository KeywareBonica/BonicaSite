-- === SUPABASE MIGRATION - WITH VIEW FIX ===
-- This script handles the view dependency issue

-- === STEP 1: Create ENUM types with the correct values ===
DO $$
BEGIN
  -- Job Cart Status ENUM - using the values you actually have
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'job_cart_status_enum') THEN
    CREATE TYPE job_cart_status_enum AS ENUM (
      'pending',
      'accepted',           -- This is the value you actually have
  ...
  -- Quotation Status ENUM - using common values
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'quotation_status_enum') THEN
    CREATE TYPE quotation_status_enum AS ENUM (
      ...
      'rejected',
      'withdrawn',
      'submitted',  -- Common value that might exist
      'confirmed'   -- Another value that exists in your data
    );
  END IF;
...
-- === STEP 4: Convert quotation.quotation_status to ENUM (handle default) ===
...
-- === STEP 9: Add foreign key for accepted_quotation_id ===
DO $$
BEGIN
  IF NOT EXISTS (
...
-- === STEP 10: Add quotation_id to booking table ===
...
-- === STEP 11: Create quotation_history table ===
...
-- === STEP 12: Add unique constraint for one accepted quotation per job cart ===
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  IF (TG_OP = 'UPDATE') 
    ...
    SET accepted_quotation_id = NEW.quotation_id,
        job_cart_status = 'accepted'  -- Using 'accepted' to match your data
...
-- === STEP 16: Set up Row Level Security for quotation_history ===
...
-- === STEP 18: Recreate the quotation_with_files view ===
-- Note: You may need to adjust this view definition based on your original view
CREATE VIEW public.quotation_with_files AS
SELECT 
  q.*,
  qf.quotation_file_path,
  qf.quotation_file_name
...
) qf ON q.quotation_id = qf.quotation_id;
